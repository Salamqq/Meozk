...
ask_ques = "**اهلا بك عزيزي في قسم استخراج الجلسات**"
buttons_ques = [
    [
        InlineKeyboardButton("‹ استخراج جلسه ›", callback_data="pyrogram"),
    ],
]
...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    if is_bot:
        ty += " ʙᴏᴛ"
    await msg.reply(f"**انت الان سوف تستخرج جلسه بايروجرام اصدار 2.0.106**")
    user_id = msg.chat.id
    api_id_msg = await bot.ask(user_id, "**♪ ارسل الان : api_id الخاص بالحساب \n يمكنك ارسال تخطي لتخطي هذه الخطوه**", filters=filters.text)
    ...
    if not is_bot:
        await msg.reply("**جاري ارسال الكود الى حسابك .**")
    else:
    ...
    try:
        ...
        await msg.reply("**لقد تاخرت في ارسال الكود\nقم باستخراج جلسه مره اخرى**", reply_markup=InlineKeyboardMarkup(gen_button))
        return
    if not is_bot:
        ...
        try:
            ...
            try:
                ...
                await two_step_msg.reply("» كلمه السر غير صحيحه\n\nقم باستخراج جلسه مره اخرى.", quote=True, reply_markup=InlineKeyboardMarkup(gen_button))
                return
    else:
        if telethon:
